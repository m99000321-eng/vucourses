          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gradient-to-r from-purple-900 via-brand-purple to-slate-900 text-white p-4 sm:p-6 rounded-2xl shadow-lg relative overflow-hidden animate-fade-in-up">
            <div className="relative z-10 space-y-2 max-w-2xl">
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 bg-brand-orange text-white text-[11px] font-extrabold rounded-full uppercase tracking-wider">VU. COURSES PLATFORM</span>
                <span className="text-xs text-purple-200 font-semibold">{t('webDevelopment')}</span>
              </div>
              <h1 className="text-xl sm:text-2xl md:text-3xl font-black">{selectedCourse?.title || t('courseTitle')}</h1>
              <p className="text-xs text-purple-100/90 leading-relaxed">{selectedCourse?.description || t('courseDescription')}</p>
              <div className="flex items-center gap-4 pt-2 text-xs text-purple-200">
                <div className="flex items-center gap-1"><User className="w-4 h-4 text-brand-orange" /><span>{t('instructor')}: {selectedCourse?.instructor.name || 'م. أحمد محمود'}</span></div>
                <div className="flex items-center gap-1"><Clock className="w-4 h-4 text-amber-400" /><span>{t('duration')}: {selectedCourse?.duration || '16 ساعة'}</span></div>
                <div className="flex items-center gap-1"><Star className="w-4 h-4 text-amber-400 fill-current" /><span>{t('rating')}: {selectedCourse?.rating || 4.9}</span></div>
              </div>
            </div>
                  ...
                  return (
                    <div key={lvl.id} className="relative group flex items-center">
                      <button
                        ...
                        className={`px-5 py-2.5 rounded-xl text-xs font-black transition flex items-center gap-2 whitespace-nowrap ${isActive ? 'bg-brand-purple text-white shadow-md shadow-brand-purple/20' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                      >
              ...
              {!activeLevel || activeLevel.lessons.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {activeLevel.lessons.map((lesson, index) => {
                    ...
                    return (
                      <div key={lesson.id} className={`bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-2xl p-4 sm:p-5 hover:border-brand-purple/50 transition shadow-sm flex flex-col justify-between space-y-3 card-hover animate-fade-in-up stagger-${index + 1}`}>
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className={`px-3 py-1 rounded-md text-[10px] font-black tracking-wider uppercase text-white ${isVideo ? 'bg-brand-purple' : isPdf ? 'bg-brand-orange' : 'bg-emerald-600'}`}>{lesson.type}</span>
                            <span className="text-[11px] text-slate-400 font-semibold">{lesson.duration}</span>
                          </div>

                          <h4 className="text-sm font-extrabold text-slate-900 dark:text-slate-100 leading-snug">{lesson.title}</h4>
                          <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">{isVideo ? t('videoDescription') : isPdf ? t('pdfDescription') : t('testDescription')}</p>
                        </div>
                        ...
                        <button
                          ...
                          className={`w-full py-2.5 rounded-xl text-xs font-bold text-white shadow transition flex items-center justify-center gap-2 ${isVideo ? 'bg-brand-purple hover:bg-brand-purple-hover' : isPdf ? 'bg-brand-orange hover:bg-brand-orange-hover' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                        >
                          {isVideo ? (
                            <><Play className="w-4 h-4 fill-current" />{t('openVideo')} [Open]</>
                          ) : isPdf ? (
                            <><FileText className="w-4 h-4" />{t('openPDF')} [Open]</>
                          ) : (
                            <><HelpCircle className="w-4 h-4" />{t('startQuiz')} [Open]</>
                          )}
                        </button>
                      </div>
                    )
